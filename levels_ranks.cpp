...
void LoadConfig()
{
...

    {
        ...
        {
            g_pUtils->ErrorLog("[%s] Failed to load levels ranks config 'addons/config/levels_ranks/settings.ini'",
                               g_PLAPI->GetLogTag());
            std::string sBuffer = "meta unload " + std::to_string(g_PLID);
            engine->ServerCommand(sBuffer.c_str());
            return;
...
bool CheckStatus(int iSlot)
{
    ...
    return (engine->IsClientFullyAuthenticated(iSlot) && g_iPlayerInfo[iSlot].bInitialized) || (g_iPlayerInfo[iSlot].
        bInitialized = false);
}
...
void SaveDataPlayer(int iSlot, bool bDisconnect = false)
{
    if (CheckStatus(iSlot))
    {
		...
		VALUES ('%s', %i, '%s', %i, %i, %i, %i, %i, %i, %i, %i, %i, %i, %i, %i);", g_sTableName,
                        g_iPlayerInfo[iSlot].szAuth.c_str(), g_iPlayerInfo[iSlot].iStats[ST_EXP],
                        g_pConnection->Escape(engine->GetClientConVarValue(iSlot, "name")).c_str(),
                        g_iPlayerInfo[iSlot].iStats[ST_RANK], g_iPlayerInfo[iSlot].iStats[ST_KILLS],
                        g_iPlayerInfo[iSlot].iStats[ST_DEATHS], g_iPlayerInfo[iSlot].iStats[ST_SHOOTS],
                        g_iPlayerInfo[iSlot].iStats[ST_HITS], g_iPlayerInfo[iSlot].iStats[ST_HEADSHOTS],
                        g_iPlayerInfo[iSlot].iStats[ST_ASSISTS], g_iPlayerInfo[iSlot].iStats[ST_ROUNDSWIN],
                        g_iPlayerInfo[iSlot].iStats[ST_ROUNDSLOSE], iTime - g_iPlayerInfo[iSlot].iStats[ST_PLAYTIME],
                        iTime, g_Settings[LR_OnlineID]);
        ...
        if (bDisconnect)
        {
            g_SMAPI->Format(szQuery, sizeof(szQuery), "UPDATE `%s` SET `online` = 0 WHERE `steam` = '%s';",
                            g_sTableName, g_iPlayerInfo[iSlot].szAuth.c_str());
...
LIMIT 1;", g_sTableName, g_iPlayerInfo[iSlot].iStats[ST_EXP], g_sTableName,
                            iTime - g_iPlayerInfo[iSlot].iStats[ST_PLAYTIME]);
            g_pConnection->Query(szQuery, [iSlot](ISQLQuery* test)
            {
                ...
                if (results->FetchRow())
                {
                    ...
                    if (iOldPlaceInTop)
                    {
                        g_iPlayerInfo[iSlot].iSessionStats[ST_PLACEINTOP] += iOldPlaceInTop - g_iPlayerInfo[iSlot].
                            iStats[ST_PLACEINTOP];
                    }

                    if (iOldPlaceInTopTime)
                    {
                        g_iPlayerInfo[iSlot].iSessionStats[ST_PLACEINTOPTIME] += iOldPlaceInTopTime - g_iPlayerInfo[
                            iSlot].iStats[ST_PLACEINTOPTIME];
                    }

                    g_pLRApi->SendOnPlayerPosInTopHook(iSlot, g_iPlayerInfo[iSlot].iStats[ST_PLACEINTOP],
                                                       g_iPlayerInfo[iSlot].iStats[ST_PLACEINTOPTIME]);
                }
            });
        }
    }
...
void CheckAllowStatistic(bool bRoundStart = false)
{
    ...
    for (int i = 0; i < 64; i++)
    {
        ...
        if (pPlayerController && pPlayerController->m_steamID() > 0 && pPlayerController->m_iTeamNum() > 1 &&
            pPlayerController->m_hPawn() && pPlayerController->m_hPlayerPawn())
        {
            iPlayers++;
        }
    }
    ...
    if (g_Settings[LR_ShowSpawnMessage] && bRoundStart)
    {
        for (int i = 0; i < 64; i++)
        {
            ...
            if (pPlayerController && pPlayerController->m_steamID() > 0)
            {
                if (bWarningMessage)
                {
                    ClientPrint(i, g_vecPhrases[std::string("RoundStartCheckCount")].c_str(), iPlayers,
                                g_Settings[LR_MinplayersCount]);
                }
        ...
        if (iMaxRanks)
        {
            ...
            if (iRank != iOldRank)
            {
                ...
                if (bActive)
                {
                    ...
                    g_SMAPI->Format(sRankName, sizeof(sRankName), "%s",
                                    g_vecPhrases[g_hRankNames[iRank ? iRank - 1 : iRank]].c_str());
                    ...
                    if (g_Settings[LR_ShowAllLevelUpMessage + !bIsUp])
                    {
                        for (int i = 0; i < 64; i++)
                        {
                            ...
                            {
                                ClientPrint(i, g_vecPhrases[std::string(bIsUp ? "LevelUpAll" : "LevelDownAll")].c_str(),
                                            engine->GetClientConVarValue(iSlot, "name"), sRankName);
                            }
                        }
                    }
...
void OverAllTopPlayers(int iSlot, bool bPlaytime = true)
{
    ...
    if (CheckStatus(iSlot))
    {
        ...
        g_SMAPI->Format(sQuery, sizeof(sQuery),
                        "SELECT `name`, `steam`, %s FROM `%s` WHERE `lastconnect` ORDER BY %.10s DESC LIMIT %i;",
                        sTable[bPlaytime], g_sTableName, sTable[bPlaytime], g_Settings[LR_TopCount]);
        g_pConnection->Query(sQuery, [iSlot, bPlaytime](ISQLQuery* test)
        {
            ...
            g_SMAPI->Format(sBuffer, sizeof(sBuffer), "%s | %s", g_sPluginTitle,
                            g_vecPhrases[std::string(sFrase)].c_str());
            g_pMenus->SetTitleMenu(hMenu, sBuffer);
            g_SMAPI->Format(sFrase, sizeof(sFrase),
                            bPlaytime ? "OverAllTopPlayersTime_Slot" : "OverAllTopPlayersExp_Slot");
...
void ResetMenuHandle(const char* szBack, const char* szFront, int iItem, int iSlot)
{
    if (iItem < 7)
    {
        ...
        case 1:
            {
                SetClientCookie(iSlot, "ResetMyStatsCooldown",
                                std::to_string(std::time(0) + g_Settings[LR_ResetMyStatsCooldown]).c_str());
...
void MyStatsSession(int iSlot)
{
    ...
    char sBuff[32];
    g_SMAPI->Format(sBuff, sizeof(sBuff), "%s%i", (g_iPlayerInfo[iSlot].iSessionStats[ST_EXP] > 0) ? "+" : "",
                    g_iPlayerInfo[iSlot].iSessionStats[ST_EXP]);

    g_SMAPI->Format(sText, sizeof(sText), g_vecPhrases[std::string("MyStatsSessionInfoExp")].c_str(), sBuff);
    g_pMenus->AddItemMenu(hMenu, "", sText, ITEM_DISABLED);
    g_SMAPI->Format(sBuff, sizeof(sBuff), "%s%i", (g_iPlayerInfo[iSlot].iSessionStats[ST_PLACEINTOP] > 0) ? "+" : "",
                    g_iPlayerInfo[iSlot].iSessionStats[ST_PLACEINTOP]);
    g_SMAPI->Format(sText, sizeof(sText), g_vecPhrases[std::string("MyStatsSessionInfoPlace")].c_str(), sBuff);
    g_pMenus->AddItemMenu(hMenu, "", sText, ITEM_DISABLED);
    g_SMAPI->Format(sText, sizeof(sText), g_vecPhrases[std::string("MyStatsInfoPlayed")].c_str(), iPlayTime / 3600,
                    iPlayTime / 60 % 60, iPlayTime % 60);
    g_pMenus->AddItemMenu(hMenu, "", sText, ITEM_DISABLED);
    ...
    g_pMenus->AddItemMenu(hMenu, "", sText, ITEM_DISABLED);
    g_SMAPI->Format(sText, sizeof(sText), g_vecPhrases[std::string("MyStatsInfoAssists")].c_str(),
                    g_iPlayerInfo[iSlot].iSessionStats[ST_ASSISTS]);
    g_pMenus->AddItemMenu(hMenu, "", sText, ITEM_DISABLED);
    g_SMAPI->Format(sText, sizeof(sText), g_vecPhrases[std::string("MyStatsInfoHeadshots")].c_str(), iHeadshots,
                    RoundToCeil(100.0 / (iKills ? iKills : 1) * iHeadshots));
    g_pMenus->AddItemMenu(hMenu, "", sText, ITEM_DISABLED);
    g_SMAPI->Format(sText, sizeof(sText), g_vecPhrases[std::string("MyStatsInfoKDR")].c_str(),
                    iKills / (iDeaths ? float(iDeaths) : 1.0));
    g_pMenus->AddItemMenu(hMenu, "", sText, ITEM_DISABLED);
    g_SMAPI->Format(sText, sizeof(sText), g_vecPhrases[std::string("MyStatsInfoAccuracy")].c_str(),
                    RoundToCeil(100.0 / (iShots ? float(iShots) : 1.0) * g_iPlayerInfo[iSlot].iSessionStats[ST_HITS]));
    g_pMenus->AddItemMenu(hMenu, "", sText, ITEM_DISABLED);
    g_SMAPI->Format(sText, sizeof(sText), g_vecPhrases[std::string("MyStatsInfoWinRate")].c_str(),
                    RoundToCeil(100.0 / (iRoundsAll ? float(iRoundsAll) : 1.0) * iRoundsWin));
    g_pMenus->AddItemMenu(hMenu, "", sText, ITEM_DISABLED);
...
void MyStats(int iSlot)
{
    ...
    g_pMenus->AddItemMenu(hMenu, "", sText, ITEM_DISABLED);
    g_SMAPI->Format(sText, sizeof(sText), g_vecPhrases[std::string("MyStatsInfoAssists")].c_str(),
                    g_iPlayerInfo[iSlot].iStats[ST_ASSISTS]);
    g_pMenus->AddItemMenu(hMenu, "", sText, ITEM_DISABLED);
    g_SMAPI->Format(sText, sizeof(sText), g_vecPhrases[std::string("MyStatsInfoHeadshots")].c_str(), iHeadshots,
                    RoundToCeil(100.0 / (iKills ? iKills : 1) * iHeadshots));
    g_pMenus->AddItemMenu(hMenu, "", sText, ITEM_DISABLED);
    ...
    if (g_Settings[LR_ShowResetMyStats])
    {
        ...
        else
        {
            ...
            g_SMAPI->Format(sText, sizeof(sText), g_vecPhrases[std::string("MyStatsResetCooldown")].c_str(),
                            iCooldown / 3600, iCooldown / 60 % 60);
...
void MenuValues(int iSlot, int iTarget)
{
    ...
    char sText[128];
    g_SMAPI->Format(sText, sizeof(sText), "%s (%i)", g_pPlayers->GetPlayerName(iTarget),
                    g_iPlayerInfo[iTarget].iStats[ST_EXP]);
    Menu hMenu;
    ...
    g_pMenus->SetCallback(hMenu, [iTarget](const char* szBack, const char* szFront, int iItem, int iSlot)
    {
        if (iItem < 7)
        {
            ...
            if (NotifClient(iTarget, iValue, iValue > 0 ? "AdminGive" : "AdminTake", true))
            {
                ...
                ClientPrint(iSlot, g_vecPhrases["ExpChange"].c_str(), g_pPlayers->GetPlayerName(iTarget), iExp,
                            iValue > 0 ? "+" : "", iValue);
                g_pUtils->LogToFile("levels_ranks", "%s give/take %i exp to %s", g_pPlayers->GetPlayerName(iSlot),
                                    iValue, g_pPlayers->GetPlayerName(iTarget));
            }
...
void GiveTakeValue(int iSlot)
{
    ...
    for (int i = 0; i < 64; i++)
    {
        ...
        {
            char sText[128];
            g_SMAPI->Format(sText, sizeof(sText), "%s (%i)", g_pPlayers->GetPlayerName(i),
                            g_iPlayerInfo[i].iStats[ST_EXP]);
...
void OnLRMenu(int iSlot)
{
    ...
    g_SMAPI->Format(sText, sizeof(sText), g_vecPhrases[std::string("MainMenuRank")].c_str(),
                    g_vecPhrases[g_hRankNames[iRank ? iRank - 1 : 0]].c_str());
    g_pMenus->AddItemMenu(hMenu, "", sText, ITEM_DISABLED);
    g_SMAPI->Format(sText, sizeof(sText), g_vecPhrases[std::string("MainMenuExp")].c_str(), sExp);
    g_pMenus->AddItemMenu(hMenu, "", sText, ITEM_DISABLED);
    g_SMAPI->Format(sText, sizeof(sText), g_vecPhrases[std::string("MainMenuPlace")].c_str(),
                    g_iPlayerInfo[iSlot].iStats[ST_PLACEINTOP], g_iDBCountPlayers);
    g_pMenus->AddItemMenu(hMenu, "", sText, ITEM_DISABLED);
...
CON_COMMAND_F(mm_lvl_give_admin, "Give admin", FCVAR_GAMEDLL)
{
    if (args.ArgC() > 1 && containsOnlyDigits(args[1]))
    {
        ...
        for (int i = 0; i < 64; i++)
        {
            ...
            if (m_steamID == std::stoll(args[1]) || std::stoll(args[1]) == i || std::stoll(args[1]) == engine->
                GetClientXUID(i))
            {
                bFound = true;
                iSlot = i;
        ...
        if (bFound)
        {
            ...
            META_CONPRINT("[LR] Player not found\n");
    }
    else
        META_CONPRINT("[LR] Usage: mm_lvl_give_admin <userid|steamid>\n");
}
...
CON_COMMAND_F(mm_lvl_reset, "Reset player statistics", FCVAR_GAMEDLL)
{
    if (args.ArgC() > 1)
    {
        ...
        if (iType != -1)
        {
            ...
            case 1:
                {
                    g_SMAPI->Format(sBuffer, sizeof(sBuffer), "UPDATE `%s` SET `value` = %i, `rank` = 0;", g_sTableName,
                                    g_Settings[LR_TypeStatistics] ? 1000 : g_Settings[LR_StartPoints]);
                    break;
                }

            case 2:
                {
                    g_SMAPI->Format(sBuffer, sizeof(sBuffer),
                                    "UPDATE `%s` SET `kills` = 0, `deaths` = 0, `shoots` = 0, `hits` = 0, `headshots` = 0, `assists` = 0, `round_win` = 0, `round_lose` = 0;",
                                    g_sTableName);
                    break;
                }
            }
...
bool LR::Load(PluginId id, ISmmAPI* ismm, char* error, size_t maxlen, bool late)
{
    ...
    SH_ADD_HOOK_MEMFUNC(IServerGameClients, ClientPutInServer, g_pSource2GameClients, this, &LR::OnClientPutInServer,
                        true);
    SH_ADD_HOOK_MEMFUNC(IServerGameClients, ClientDisconnect, g_pSource2GameClients, this, &LR::OnClientDisconnect,
                        true);
...
{
    SH_REMOVE_HOOK_MEMFUNC(IServerGameClients, ClientPutInServer, g_pSource2GameClients, this, &LR::OnClientPutInServer,
                           true);
    SH_REMOVE_HOOK_MEMFUNC(IServerGameClients, ClientDisconnect, g_pSource2GameClients, this, &LR::OnClientDisconnect,
                           true);
...
void LR::OnClientDisconnect(CPlayerSlot slot, ENetworkDisconnectionReason reason, const char* pszName, uint64 xuid,
...
void LR::OnClientPutInServer(CPlayerSlot slot, char const* pszName, int type, uint64 xuid)
{
    ...
    g_SMAPI->Format(szQuery, sizeof(szQuery),
                    "SELECT `value`, `rank`, `kills`, `deaths`, `shoots`, `hits`, `headshots`, `assists`, `round_win`, `round_lose`, `playtime`, (SELECT COUNT(`steam`) FROM `%s` WHERE `value` >= `player`.`value` AND `lastconnect`) AS `exppos`, (SELECT COUNT(`steam`) FROM `%s` WHERE `playtime` >= `player`.`playtime` AND `lastconnect`) AS `timepos` FROM `%s` `player` WHERE `steam` = '%s' LIMIT 1;",
                    g_sTableName, g_sTableName, g_sTableName, g_iPlayerInfo[iSlot].szAuth.c_str());
    g_pConnection->Query(szQuery, [iSlot, this](ISQLQuery* test)
    {
        ...
        if (results->FetchRow())
        {
            ...
            char szQuery[512];
            g_SMAPI->Format(szQuery, sizeof(szQuery),
                            "UPDATE `%s` SET `online` = %i, `lastconnect` = %i WHERE `steam` = '%s';", g_sTableName,
                            g_Settings[LR_OnlineID], std::time(0), g_iPlayerInfo[iSlot].szAuth.c_str());
            g_pConnection->Query(szQuery, [iSlot, this](ISQLQuery* test)
            {
            });
        ...
        else
        {
            char szQuery[512];
            g_SMAPI->Format(szQuery, sizeof(szQuery),
                            "INSERT INTO `%s` (`steam`, `name`, `value`, `lastconnect`) VALUES ('%s', '%s', %i, %i);",
                            g_sTableName, g_iPlayerInfo[iSlot].szAuth.c_str(),
                            g_pConnection->Escape(engine->GetClientConVarValue(iSlot, "name")).c_str(),
                            g_Settings[LR_TypeStatistics] ? 1000 : g_Settings[LR_StartPoints], std::time(0));
            g_pConnection->Query(szQuery, [iSlot, this](ISQLQuery* test)
            {
                if (!g_iPlayerInfo[iSlot].bInitialized)
                {
                    ...
                    g_iDBCountPlayers++;
                    g_iPlayerInfo[iSlot].bInitialized = true;
                    g_iPlayerInfo[iSlot].iStats[ST_EXP] = g_Settings[LR_TypeStatistics]
                                                              ? 1000
                                                              : g_Settings[LR_StartPoints];
...
void OnOtherEvents(const char* sName, IGameEvent* event, bool bDontBroadcast)
{
    ...
    if (iSlot < 64)
    {
        CBasePlayerController* pPlayerController = static_cast<CBasePlayerController*>(event->
            ...
            for (int i = 0; i < 64; i++)
            {
                ...
                if (pPlayerController && pPlayerController->m_hPawn() && pPlayerController->m_steamID() > 0)
                {
                    ...
                    if (g_Settings[LR_ShowUsualMessage] == 2)
                    {
                        ...
                        ClientPrint(i, g_vecPhrases[std::string("RoundExpResultAll")].c_str(),
                                    g_iPlayerInfo[i].iStats[ST_EXP]);
        ...
        else
        {
            ...
            else
            {
                ...
                if (!g_Settings[LR_TypeStatistics])
                {
                    ...
                    g_pLRApi->SendOnPlayerKilledPreHook(event, iExpAttacker, g_iPlayerInfo[iClient].iStats[ST_EXP],
                                                        g_iPlayerInfo[iAttacker].iStats[ST_EXP]);
                }
                else if (!bFakeClient && !bFakeAttacker)
                {
                    if (g_Settings[LR_TypeStatistics] == 1)
                    {
                        iExpAttacker = RoundToNearest(
                            float(g_iPlayerInfo[iClient].iStats[ST_EXP]) / g_iPlayerInfo[iAttacker].iStats[ST_EXP] *
                            5.0);
                        ...
                        if ((iExpVictim = RoundToNearest(iExpAttacker * (g_SettingsStats[LR_ExpKillCoefficient] / 100)))
                            < 1)
                        {
                            iExpVictim = 1;
                        }
                ...
                if (NotifClient(iAttacker, iExpAttacker, "Kill") + NotifClient(iClient, -iExpVictim, "MyDeath"))
                {
                    if (!bFakeAttacker)
                    {
                        if (event->GetBool("headshot") &&
                            NotifClient(iAttacker, g_SettingsStats[LR_ExpGiveHeadShot], "HeadShotKill"))
                        {
                    ...
                    g_pLRApi->SendOnPlayerKilledPostHook(event, iExpAttacker,
                                                         g_iPlayerInfo[iClient].iStats[ST_EXP],
                                                         g_iPlayerInfo[iAttacker].iStats[ST_EXP]);
                }
            }
        }
...
void OnBombEvent(const char* sName, IGameEvent* event, bool bDontBroadcast)
{
    ...
    if (iSlot < 64)
    {
        ...
        {
            if (g_iPlayerInfo[iSlot].bHaveBomb &&
                NotifClient(iSlot, -g_SettingsStats[LR_ExpBombDropped], "BombDropped"))
            {
                g_iPlayerInfo[iSlot].bHaveBomb = false;
            }
        }
...
void OnCleanDB()
{
    if (g_Settings[LR_CleanDB_Days])
    {
        char szQuery[256];
        g_SMAPI->Format(szQuery, sizeof(szQuery),
                        "UPDATE `%s` SET `lastconnect` = 0 WHERE `lastconnect` < %d AND `lastconnect`;", g_sTableName,
                        std::time(0) - g_Settings[LR_CleanDB_Days] * 86400);
        g_pConnection->Query(szQuery, [](ISQLQuery* test)
        {
        });
    }
...
`online` int NOT NULL DEFAULT 0 \
            ...
            g_SMAPI->Format(szQuery, sizeof(szQuery), "ALTER TABLE `%s` ADD COLUMN `online` int NOT NULL DEFAULT 0;",
                            g_sTableName);
            g_pConnection->Query(szQuery, [](ISQLQuery* test)
            {
            });

            g_SMAPI->Format(szQuery, sizeof(szQuery), "SELECT COUNT(`steam`) FROM `%s` WHERE `lastconnect` LIMIT 1;",
                            g_sTableName);
            g_pConnection->Query(szQuery, [](ISQLQuery* test)
            {
                auto results = test->GetResultSet();
                ...
                g_SMAPI->Format(sCharsetType, sizeof(sCharsetType), "%s",
                                g_Settings[LR_DB_Charset_Type] ? "_unicode_ci" : "_general_ci");
                ...
                g_SMAPI->Format(szQuery, sizeof(szQuery), "ALTER TABLE `%s` CHARACTER SET '%s' COLLATE '%s%s';",
                                g_sTableName, sCharset, sCharset, sCharsetType);
                g_pConnection->Query(szQuery, [](ISQLQuery* test)
                {
                });

                g_SMAPI->Format(szQuery, sizeof(szQuery),
                                "ALTER TABLE `%s` MODIFY COLUMN `name` varchar(%i) CHARACTER SET '%s' COLLATE '%s%s' NOT NULL default '' AFTER `steam`;",
                                g_sTableName, 128, sCharset, sCharset, sCharsetType);
                g_pConnection->Query(szQuery, [](ISQLQuery* test)
                {
                });

                g_SMAPI->Format(szQuery, sizeof(szQuery), "UPDATE `%s` SET `online` = 0 WHERE `online` = %i;",
                                g_sTableName, g_Settings[LR_OnlineID]);
                g_pConnection->Query(szQuery, [](ISQLQuery* test)
                {
                });
    ...
    g_pUtils->RegCommand(g_PLID, {"mm_lvl", "sm_lvl"}, {"!lvl", "lvl", "!дмд", "дмд"},
    ...
    g_pUtils->RegCommand(g_PLID, {"mm_rank", "sm_rank"}, {"!rank", "rank", "!кфтл", "кфтл"},
                         [](int iSlot, const char* szContent)
                         {
                             ...
                             if (g_Settings[LR_ShowRankMessage])
                             {
                                 ...
                                 for (int i = 0; i < 64; i++)
                                 {
                                     if (CheckStatus(i))
                                     {
                                         ClientPrint(i, g_vecPhrases[std::string("RankPlayer")].c_str(),
                                                     engine->GetClientConVarValue(iSlot, "name"), iPlaceInTop,
                                                     g_iDBCountPlayers, iExp, iKills, iDeaths, fKDR);
                                     }
                                 }
                             }
                             else
                             {
                                 ClientPrint(iSlot, g_vecPhrases[std::string("RankPlayer")].c_str(),
                                             engine->GetClientConVarValue(iSlot, "name"),
                                             g_iPlayerInfo[iSlot].iStats[ST_PLACEINTOP], g_iDBCountPlayers,
                                             g_iPlayerInfo[iSlot].iStats[ST_EXP], iKills, iDeaths, fKDR);
                             }
                             return false;
                         });

    g_pUtils->RegCommand(g_PLID, {"mm_session", "sm_session"}, {"!session", "session", "!ыуыышщт", "ыуыышщт"},
                         [](int iSlot, const char* szContent)
                         {
                             return false;
                         });

    g_pUtils->RegCommand(g_PLID, {"mm_toptime", "sm_toptime"}, {"!toptime", "toptime", "!ещзешьу", "ещзешьу"},
                         [](int iSlot, const char* szContent)
                         {
                             OverAllTopPlayers(iSlot);
                             return false;
    ...
    g_pUtils->RegCommand(g_PLID, {"mm_top", "sm_top"}, {"!top", "top", "!ещз", "ещз"},
                         [](int iSlot, const char* szContent)
                         {
                             OverAllTopPlayers(iSlot, false);
                             return false;
